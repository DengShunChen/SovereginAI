#!/usr/bin/env bash
# 氣象主權 AI — M2 Mac 本機 LoRA 微調（使用 Apple MLX）
# 自專案根目錄執行：./scripts/train/train_lora_mlx.sh
# 或：python -m mlx_lm lora --help 查看完整參數

set -e
cd "$(dirname "$0")/../.."

DATA_DIR="${DATA_DIR:-data/processed/for_training}"
ADAPTER_DIR="${ADAPTER_DIR:-models/sovereign-weather-lora}"
MODEL="${MODEL:-mlx-community/Qwen2.5-1.5B-Instruct-4bit}"  # 中文佳，M2 16GB 可跑；8GB 可改 Qwen2.5-Coder-0.5B-Instruct-4bit
...
# M2 Mac 建議：batch_size 2–4、小模型（0.5B–1.5B）
